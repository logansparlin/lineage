...
const containerStyles = cva('px-20 md:px-0 w-full md:w-[90vw] h-screen py-80 flex flex-col gap-y-20', {
  variants: {
    size: {
  ...
  return (
    <div className={containerStyles({ size: items.length <= 2 ? '2' : 'grid' })}>
      {items.map((item, index) => (
        <div key={item._key} className={itemStyles({ size: items.length <= 2 ? '2' : items.length === 3 ? '3' : 'grid', hasCaption: !!item.caption })}>
          <Media
            {...item}
