...
export const PerspectiveImageClone = ({ url, slug, aspectRatio, index, stencil }) => {
  ...
  useLayoutEffect(() => {
    ...
    const updateDimensions = () => {
      ...
      const planeHeight = bounds.height / viewport.factor;
      const heightScale = planeHeight / viewport.height;
      const widthScale = planeHeight * 1.65 / viewport.width;
      const planeWidth = planeHeight * 1.65;

      const planeTop = (container.clientTop / viewport.factor) * heightScale;
      
      setDimensions({ width: 5 * widthScale, height: 5 * heightScale, top: planeTop });
    };
  ...
  return (
    <group position={[0, dimensions.top, 0]}>
      <Image ref={imageRef} url={url} scale={[dimensions.width, dimensions.height, 1]} radius={0.025} transparent {...stencil} />
    </group>
  )
}