export const StepThreeIllo = () => {
  return (
    <div className="col-span-full w-full px-60 flex justify-center">
      <svg className="max-w-full" width="1207" height="1060" viewBox="0 0 1207 1060" fill="none" xmlns="http://www.w3.org/2000/svg">
        <path d="M0.725694 818.601H275.038V1058.81H0.725694V818.601ZM276.49 1058.14V818.292L497.101 689.963V903.855L276.49 1058.14ZM2.75933 817.149L228.785 689.427H495.136L275.568 817.149H2.75933Z" fill="black" stroke="#505050" strokeWidth="1.45139"/>
        <path opacity="0.2" d="M353.265 130.596H627.517V1058.24H353.265V130.596ZM628.968 1057.57V130.288L849.53 1.98698V903.32L628.968 1057.57ZM355.298 129.145L581.273 1.45112H847.566L628.047 129.145H355.298Z" fill="url(#paint0_linear_22_358)" stroke="url(#paint1_linear_22_358)" strokeWidth="1.45107"/>
        <path d="M353.265 130.399H627.577V1059.09H353.265V130.399ZM629.029 1058.42V130.09L849.64 1.76096V904.135L629.029 1058.42ZM355.298 128.947L581.324 1.22496H847.675L628.107 128.947H355.298Z" stroke="url(#paint2_linear_22_358)" strokeWidth="1.45139"/>
        <path d="M709.003 571.14H983.316V1058.81H709.003V571.14ZM984.767 1058.14V570.831L1205.38 442.502V903.855L984.767 1058.14ZM711.037 569.688L937.062 441.966H1203.41L983.845 569.688H711.037Z" fill="black" stroke="#505050" strokeWidth="1.45139"/>
        <circle opacity="0.2" cx="600.844" cy="356.155" r="155.676" fill="url(#paint3_linear_22_358)"/>
        <circle opacity="0.3" cx="601.498" cy="355.504" r="106.574" fill="url(#paint4_linear_22_358)"/>
        <defs>
        <linearGradient id="paint0_linear_22_358" x1="601.397" y1="0.725586" x2="601.397" y2="1058.96" gradientUnits="userSpaceOnUse">
        <stop stopColor="#01C2FF"/>
        <stop offset="1" stopColor="#087EEE"/>
        </linearGradient>
        <linearGradient id="paint1_linear_22_358" x1="601.397" y1="0.725586" x2="601.397" y2="1058.96" gradientUnits="userSpaceOnUse">
        <stop stopColor="#01C2FF"/>
        <stop offset="1" stopColor="#087EEE"/>
        </linearGradient>
        <linearGradient id="paint2_linear_22_358" x1="601.452" y1="0.499268" x2="601.452" y2="1059.81" gradientUnits="userSpaceOnUse">
        <stop stopColor="#01C2FF"/>
        <stop offset="1" stopColor="#087EEE"/>
        </linearGradient>
        <linearGradient id="paint3_linear_22_358" x1="600.844" y1="200.479" x2="600.844" y2="511.831" gradientUnits="userSpaceOnUse">
        <stop stopColor="#01C2FF"/>
        <stop offset="1" stopColor="#087EEE"/>
        </linearGradient>
        <linearGradient id="paint4_linear_22_358" x1="601.498" y1="248.93" x2="601.498" y2="462.078" gradientUnits="userSpaceOnUse">
        <stop stopColor="#01C2FF"/>
        <stop offset="1" stopColor="#087EEE"/>
        </linearGradient>
        </defs>
      </svg>
    </div>
  )
}